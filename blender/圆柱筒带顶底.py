...
height = 1.0            # 筒壁净高度（不包含底板和顶盖厚度）
bottom_thickness = 0.01 # 底板厚度
top_thickness = 0.01    # 顶盖厚度
segments = 50          # 圆周分段数
...
if thickness == 0:
    ...
    for i in range(segments):
        next_i = (i + 1) % segments
        v0 = top_verts[next_i]  # 注意顺序，确保法向量向下（封闭容器内表面）
        v1 = top_verts[i]
        v2 = bm.verts.new((0, 0, height))  # 顶面中心点
    ...
    # 计算总高度（净高度 + 底板厚度 + 顶盖厚度）
    total_height = height + bottom_thickness + top_thickness
    ...
    for i in range(segments):
        ...
        # 内圆顶点（从底板上表面到顶盖下表面）
        inner_top_verts.append(bm.verts.new((inner_radius * cos_a, inner_radius * sin_a, total_height - top_thickness)))
    ...
    # 生成顶盖（实心圆盘，厚度为top_thickness）
    # 顶盖顶面中心点和顶面内圆顶点
    top_center_top = bm.verts.new((0, 0, total_height))
    top_center_bottom = bm.verts.new((0, 0, total_height - top_thickness))
    inner_top_top_verts = []
    ...
    for i in range(segments):
        ...
        cos_a = math.cos(angle)
        sin_a = math.sin(angle)
        inner_top_top_verts.append(bm.verts.new((inner_radius * cos_a, inner_radius * sin_a, total_height)))
    ...
    for i in range(segments):
        next_i = (i + 1) % segments
        v0 = top_center_top
        v1 = inner_top_top_verts[i]
        v2 = inner_top_top_verts[next_i]
        bm.faces.new([v0, v1, v2])
    
    # 顶盖顶面环形部分
    for i in range(segments):
        next_i = (i + 1) % segments
        v0 = inner_top_top_verts[next_i]
        v1 = inner_top_top_verts[i]
        v2 = outer_top_verts[i]
        v3 = outer_top_verts[next_i]
        bm.faces.new([v0, v1, v2, v3])
    ...
    for i in range(segments):
        next_i = (i + 1) % segments
        v0 = top_center_bottom
        v1 = inner_top_verts[next_i]
        v2 = inner_top_verts[i]
        bm.faces.new([v0, v1, v2])
    
    # 顶盖底面环形部分
    for i in range(segments):
        next_i = (i + 1) % segments
        v0 = inner_top_verts[i]
        v1 = inner_top_verts[next_i]
        v2 = outer_top_verts[next_i]
    ...
    for i in range(segments):
        next_i = (i + 1) % segments
        v0 = inner_top_verts[next_i]
        v1 = inner_top_top_verts[next_i]
        v2 = inner_top_top_verts[i]
        v3 = inner_top_verts[i]
        bm.faces.new([v0, v1, v2, v3])
    ...
    print("已生成实体圆柱筒（带顶底），内半径=%.3f，壁厚=%.3f，净高=%.3f，底厚=%.3f，顶厚=%.3f，总高=%.3f，位置=%s，分段数=%d" % (inner_radius, thickness, height, bottom_thickness, top_thickness, total_height, position, segments))